...
public class PropertyEventMapper {
  ...
  private ApartmentEvent buildApartmentEvent(Property property) {
    ...
    ApartmentEvent event =
        ApartmentEvent.builder()
            .apartmentType(apartment.getApartmentType())
            .commonPhysicalDetailsDto(
                mapper.buildCommonPhysicalDetailsDto(apartment.getCommonPhysicalDetails()))
  ...
  private CommercialEvent buildCommercialEvent(Property property) {
    ...
    CommercialEvent event =
        CommercialEvent.builder()
            .commonPhysicalDetailsDto(
                mapper.buildCommonPhysicalDetailsDto(commercial.getCommonPhysicalDetails()))
            .commercialPhysicalDetailsDto(
                mapper.buildCommercialPhysicalDetailsDto(commercial.getCommercialPhysicalDetails()))
  ...
  private LandPlotEvent buildLandPlotEvent(Property property) {
    ...
    LandPlotEvent event =
        LandPlotEvent.builder()
            .commonLandDetailsDto(mapper.buildCommonLandDetailsDto(landPlot.getCommonLandDetails()))
            .additionalBuildings(
                mapper.getAdditionalBuildingsIds(landPlot.getAdditionalBuildings()))
            .build();
    fillGeneralFields(event, property);
    return event;
  }
