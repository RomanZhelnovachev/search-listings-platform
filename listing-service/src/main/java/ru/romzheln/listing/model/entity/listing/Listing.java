...
public class Listing {
  ...
  @SequenceGenerator(name = "listing_seq", sequenceName = "listing_seq", allocationSize = 1)
  private Long id;
  ...
  public void assignPromotion(Long promotionId) {
    ...
    validateEditable();
    if (Objects.equals(this.promotionId, promotionId)) {
      throw new ChangeListingException(id);
    }
  ...
  public void removeMortgagePrograms(Set<Long> programs) {
    ...
    if (!mortgageProgramIds.containsAll(programs)) {
      throw new MortgageProgramsRemovedException(
          "Невозможно удалить несуществующие ипотечные программы");
    }
  ...
  public Set<Long> addImages(Set<Long> images) {
    Set<Long> newImages =
        images.stream().filter(image -> !this.imageIds.contains(image)).collect(Collectors.toSet());
    switch (status) {
  ...
  public void removeImages(Set<Long> images) {
    validateEditable();
    Set<Long> missingImages =
        images.stream().filter(e -> !this.imageIds.contains(e)).collect(Collectors.toSet());
  ...
  public void archive() {
    if (this.status != ListingStatus.PUBLISHED) {
      throw new ListingArchiveException(id, this.status);
    }
  ...
  public void approve() {
    if (status != ListingStatus.CREATED) {
      throw new ListingAlreadyApproved(id, this.status);
    }
