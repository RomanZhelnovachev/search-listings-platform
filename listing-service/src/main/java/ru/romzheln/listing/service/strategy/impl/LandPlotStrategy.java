...
public class LandPlotStrategy extends AbstractPropertyStrategy {
  ...
  public Property create(CreatePropertyRequest request) {
    ...
    CreateLandPlotRequest landPlotRequest =
        ClassCastUtil.requireType(request, CreateLandPlotRequest.class);
    LandUse landUse = resolver.getLandUse(landPlotRequest.getCommonLandDetailsDto().landUse());
    CommonLandDetails commonLandDetails =
        mapper.buildCommonLandDetails(landPlotRequest.getCommonLandDetailsDto(), landUse);
    LandPlot landPlot =
        LandPlot.builder()
            .property(property)
            .commonLandDetails(commonLandDetails)
            .additionalBuildings(
                resolver.getAllAdditionalBuildingsById(landPlotRequest.getAdditionalBuildings()))
            .build();
  ...
  public Property update(Long id, UpdatePropertyRequest request) {
    ...
    UpdateLandPlotRequest landPlotRequest =
        ClassCastUtil.requireType(request, UpdateLandPlotRequest.class);
    ...
    if (landPlotRequest.getCommonLandDetailsDto() != null) {
      ...
      mapper.updateCommonLandDetails(
          landPlot.getCommonLandDetails(), landPlotRequest.getCommonLandDetailsDto(), landUse);
    }
    if (landPlotRequest.getAdditionalBuildings() != null) {
      landPlot.setAdditionalBuildings(
          resolver.getAllAdditionalBuildingsById(landPlotRequest.getAdditionalBuildings()));
    }
    return property;
  }
