...
public class ReadModelResolver {
  ...
  public void updateReadModel(
      ListingKey key, PropertyType type, ListingUpdatedEvent event, Instant time) {
  ...
  public void updateProperty(ListingKey key, PropertyType type, PropertyEvent event, Instant time) {
    ...
    switch (type) {
      case APARTMENT -> {
        ...
        if (changedRegion) {
          apartmentUpdateRegion(key, model, newRegion);
        }
      }
      case COMMERCIAL -> {
        ListingCommercialReadModel model = getCommercialModel(key);
        ...
        model.setUpdatedAt(time);
        if (changedRegion) {
          commercialUpdateRegion(key, model, newRegion);
        }
      }
      case HOUSE -> {
        ListingHouseReadModel model = getHouseModel(key);
        ...
        model.setUpdatedAt(time);
        if (changedRegion) {
          houseUpdateRegion(key, model, newRegion);
        }
      }
      case LAND_PLOT -> {
        ListingLandPlotReadModel model = getLandPlotModel(key);
        ...
        model.setUpdatedAt(time);
        if (changedRegion) {
          landPlotUpdateRegion(key, model, newRegion);
        }
      }
    }
  }
  ...
  private ListingCommercialReadModel getCommercialModel(ListingKey key) {
    return commercialRepository
        .findById(key)
        .orElseThrow(() -> new ReadModelNotFoundException(key));
  }
  ...
  private void apartmentUpdateRegion(
      ListingKey key, ListingApartmentReadModel oldModel, String newRegion) {
    ListingApartmentReadModel newModel =
        factory.updateRegionApartment(newRegion, oldModel);
    apartmentRepository.save(newModel);
    apartmentRepository.deleteById(key);
  }

  private void commercialUpdateRegion(ListingKey key, ListingCommercialReadModel oldModel, String newRegion){
      ListingCommercialReadModel newModel = factory.updateRegionCommercial(newRegion, oldModel);
      commercialRepository.save(newModel);
      commercialRepository.deleteById(key);
  }

    private void houseUpdateRegion(ListingKey key, ListingHouseReadModel oldModel, String newRegion){
        ListingHouseReadModel newModel = factory.updateRegionHouse(newRegion, oldModel);
        houseRepository.save(newModel);
        houseRepository.deleteById(key);
    }

    private void landPlotUpdateRegion(ListingKey key, ListingLandPlotReadModel oldModel, String newRegion){
        ListingLandPlotReadModel newModel = factory.updateRegionLandPlot(newRegion, oldModel);
        landPlotRepository.save(newModel);
        landPlotRepository.deleteById(key);
    }
}
